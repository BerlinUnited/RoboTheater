<?xml version="1.0" encoding="UTF-8" ?>
<Package name="Theater_PlayVideo" format_version="4">
    <Manifest src="manifest.xml" />
    <BehaviorDescriptions>
        <BehaviorDescription name="behavior" src="behavior_1" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="collapse" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="awake" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="video1" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="Acorrect" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="Bcorrect" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="Ccorrect" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="Dcorrect" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="CallFromFuture" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="CallFromFuture800" xar="behavior.xar" />
    </BehaviorDescriptions>
    <Dialogs />
    <Resources>
        <File name="example" src="html/vid/example.mp4" />
        <File name="Acorrect" src="html/vid/Acorrect.mp4" />
        <File name="Bcorrect" src="html/vid/Bcorrect.mp4" />
        <File name="Ccorrect" src="html/vid/Ccorrect.mp4" />
        <File name="Dcorrect" src="html/vid/Dcorrect.mp4" />
        <File name="callfromfuture" src="html/vid/callfromfuture.mp4" />
        <File name="callfromfuture800" src="html/vid/callfromfuture800.mp4" />
    </Resources>
    <Topics />
    <IgnoredPaths />
    <Translations auto-fill="en_US">
        <Translation name="translation_en_US" src="translations/translation_en_US.ts" language="en_US" />
    </Translations>
</Package>
